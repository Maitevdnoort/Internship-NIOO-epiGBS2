...
# @Date: 19-11-2020
# @function: makes ggplots of the unique reads from library 2.
library(data.table)
library(dplyr)
library(tidyverse)
library(reshape2)
...
ggplot(lib2dups, aes(fill=variable, y=value, x= Sample_names)) + 
  ...
  labs(y = 'number of unique reads', x = 'sample names') + 
  ggtitle("library 2 unique reads") +
  ...
  ggsave(paste(outputFigures,"library2_unique_reads.png",sep=""))
...
ggplot(lib2dups, aes(fill=variable, y=value, x= reorder(Sample_names, -value))) + 
  geom_bar(position="stack", stat="identity")+
  labs(y = 'number of unique reads', x = 'sample names') +
  ggtitle("library 2 From higest unique reads to lowest") +
  scale_x_discrete(guide = guide_axis(angle = 90)) +
  geom_text(aes(label = value), size = 3, hjust = 0.5, vjust = 3, position = "stack") +
  scale_fill_manual(values = c("skyblue", "cyan3", "blueviolet", "pink")) +
  ggsave(paste(outputFigures,"library2_from_higest_unique_read_to_lowest.png",sep=""))
...
ggplot(lib2dups, aes(fill=variable, y=value, x=Sample_names)) + 
  geom_bar(position="stack", stat="identity")+
  labs(y = 'number of unique reads', x = 'sample names') + 
  ggtitle("library 2 unique reads with C and D from the same sample next to each other") +
  scale_x_discrete(guide = guide_axis(angle = 90)) +
  geom_text(aes(label = value), size = 3, hjust = 0.5, vjust = 3, position = "stack") +
  scale_fill_manual(values = c("skyblue", "cyan3", "blueviolet", "pink")) +
  ggsave(paste(outputFigures,"library2_unique_reads_with_C_and_D.png",sep=""))
